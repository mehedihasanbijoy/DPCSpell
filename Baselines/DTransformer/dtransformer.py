...
def main():
    ...
    # ------------------------------
    PATH = './Checkpoints/dtransformer.pth'
    # ------------------------------
    gc.collect()
    ...
    error_types = list(sorted(list(set(df['ErrorType'].values))))
    # ---------------------
    for error_name in error_types:
        ...
        eval_df = evaluation_report(error_data, SRC, TRG, WORD, model, DEVICE)

        error_name = error_name.replace(' ', '').replace('(', '').replace(')', '')
        eval_df.to_csv(f'./Dataframes/dtransformer_{error_name}.csv', index=False)
        print('\n\n')
    # ---------------------
